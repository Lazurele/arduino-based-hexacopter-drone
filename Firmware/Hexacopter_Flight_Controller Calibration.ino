...
//3 = check rotation / vibrations for motor 3 (right rear CCW).
//4 = check rotation / vibrations for motor 4 (right rear CW).
//5 = check rotation / vibrations for motor 5 (left rear CCW).
//6 = check rotation / vibrations for motor 6 (left front CW).
//7 = check vibrations for all motors together.
...
void loop(){
  ...
  if(Serial.available() > 0){
    ...
    if(data == '2')Serial.println("Test motor 2 (right middle CW.)");
    if(data == '3')Serial.println("Test motor 3 (right rear CCW.)");
    if(data == '4')Serial.println("Test motor 4 (left rear CW.)");
    if(data == '5')Serial.println("Test motor 5 (left middle CCW.)");
    if(data == '6')Serial.println("Test motor 6 (left front CW.)");
    if(data == '7')Serial.println("Test all motors together");
    ...
    for(vibration_counter = 0; vibration_counter < 625; vibration_counter++){           //Do this loop 625 times
      ...
      esc_5 = 1000;                                                                     //Set the pulse for ESC 5 to 1000us.
      esc_6 = 1000;                                                                     //Set the pulse for ESC 6 to 1000us.
  ...
  if(data == 0 && new_function_request == false){                                       //Only start the calibration mode at first start. 
    ...
    esc_5 = receiver_input_channel_3;                                                   //Set the pulse for motor 5 equal to the throttle channel.
    esc_6 = receiver_input_channel_3;                                                   //Set the pulse for motor 6 equal to the throttle channel.
    esc_pulse_output();                                                                 //Send the ESC control pulses.
  }
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////
  //When user sends a '1, 2, 3, 4, 5, 6 or 7 test the motors.
  ////////////////////////////////////////////////////////////////////////////////////////////
  if(data == '1' || data == '2' || data == '3' || data == '4' || data == '5' || data == '6' || data == '7'){          //If motor 1, 2, 3, 4, 5 or 6 is selected by the user.
    ...
    if(new_function_request == false){                                                  //When the throttle was in the lowest position do this.
      receiver_input_channel_3 = convert_receiver_channel(3);                           //Convert the actual receiver signals for throttle to the standard 1000 - 2000us.
      if(data == '1' || data == '7')esc_1 = receiver_input_channel_3;                   //If motor 1 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_1 = 1000;                                                                //If motor 1 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '2' || data == '7')esc_2 = receiver_input_channel_3;                   //If motor 2 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_2 = 1000;                                                                //If motor 2 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '3' || data == '7')esc_3 = receiver_input_channel_3;                   //If motor 3 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_3 = 1000;                                                                //If motor 3 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '4' || data == '7')esc_4 = receiver_input_channel_3;                   //If motor 4 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_4 = 1000;                                                                //If motor 4 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '5' || data == '7')esc_5 = receiver_input_channel_3;                   //If motor 5 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_5 = 1000;                                                                //If motor 5 is not requested set the pulse for the ESC to 1000us (off).
      if(data == '6' || data == '7')esc_6 = receiver_input_channel_3;                   //If motor 6 is requested set the pulse for motor 1 equal to the throttle channel.
      else esc_6 = 1000;                                                                //If motor 6 is not requested set the pulse for the ESC to 1000us (off).
  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      for (cal_int = 0; cal_int < 2000 ; cal_int ++){                                   //Take 2000 readings for calibration.
        ...
        PORTD |= B11111100;                                                             //Set digital poort 2, 3, 4, 5, 6 and 7 high.
        delayMicroseconds(1000);                                                        //Wait 1000us.
        PORTD &= B00000011;                                                             //Set digital poort 2, 3, 4, 5, 6 and 7 low.
...
void esc_pulse_output(){
  zero_timer = micros();
  PORTD |= B11111100;                                            //Set port 2, 3, 4, 5, 6 and 7 high at once
  timer_channel_1 = esc_1 + zero_timer;                          //Calculate the time when digital port 2 is set low.
  timer_channel_2 = esc_2 + zero_timer;                          //Calculate the time when digital port 3 is set low.
  timer_channel_3 = esc_3 + zero_timer;                          //Calculate the time when digital port 4 is set low.
  timer_channel_4 = esc_4 + zero_timer;                          //Calculate the time when digital port 5 is set low.
  timer_channel_5 = esc_5 + zero_timer;                          //Calculate the time when digital port 6 is set low.
  timer_channel_6 = esc_6 + zero_timer;                          //Calculate the time when digital port 7 is set low.

  while(PORTD >= 4){                                            //Execute the loop until digital port 2 to 7 is low.
    esc_loop_timer = micros();                                   //Check the current time.
    if(timer_channel_1 <= esc_loop_timer)PORTD &= B11111011;     //When the delay time is expired, digital port 2 is set low.
    if(timer_channel_2 <= esc_loop_timer)PORTD &= B11110111;     //When the delay time is expired, digital port 3 is set low.
